...
if (isset($_POST['confirm'])) {
    try {
        $sql_delete = "DELETE FROM coupon_code WHERE id = $id";
        if ($conn->query($sql_delete) === TRUE) {
            $_SESSION['coupon_delete_msg'] = "Coupon Delete successful!";
            echo "<script>window.location.href='admin_coupon.php';</script>";
            exit;
        } else {
            echo "<script>alert('Error deleting coupon: " . $conn->error . "');</script>";
        }
    } catch (mysqli_sql_exception $e) {
        if (strpos($e->getMessage(), 'foreign key constraint')) {
            echo "<script>alert('Error deleting coupon: Cannot delete or update a parent row due to a foreign key constraint');</script>";
        } else {
            echo "<script>alert('Error deleting coupon: " . $e->getMessage() . "');</script>";
        }
    }
}
?>
...
<div class="w-[65%] mt-8 mx-auto p-12 bg-white border border-gray-200 rounded-3xl shadow dark:bg-gray-800 dark:border-gray-700">
    ...
    <form action="admin_coupon_delete.php?id=<?= $id ?>" method="post" class="flex justify-between mt-8">
