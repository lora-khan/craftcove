...
if (isset($_POST['confirm'])) {
    try {
        ...
        if ($conn->query($sql_delete) === TRUE) {
            $_SESSION['product_delete_msg'] = "Product Delete successful";
            echo "<script>window.location.href='admin_product.php';</script>";
            exit;
        } else {
            echo "<script>alert('Error while deleting Product: " . $conn->error . "');</script>";
        }
    } catch (mysqli_sql_exception $e) {
        if (strpos($e->getMessage(), 'foreign key constraint')) {
            echo "<script>alert('Error while deleting product: Cannot delete or update a parent row due to a foreign key constraint');</script>";
        } else {
            echo "<script>alert('Error while deleting product: " . $e->getMessage() . "');</script>";
        }
    }
}
?>
...
<div class="w-[65%] mt-8 mx-auto p-12 bg-white border border-gray-200 rounded-3xl shadow dark:bg-gray-800 dark:border-gray-700">
    ...
    <form action="admin_product_delete.php?id=<?= $id ?>" method="post" class="flex justify-between mt-8">
